네트워크 구축 과정에서 어려움을 겪는 경우를 대비하여, **Packet Tracer CLI에 그대로 복사하여 붙여넣기(Paste)** 할 수 있는 최종 설정 스크립트를 단계별로 구성하였습니다. 각 장비의 프롬프트 상태(`config`, `config-if` 등)를 명시하였으며, 필수 명령어인 `no shutdown`과 주석을 포함했습니다.

```
#erase startup-config 를 먼저 해주세요.
```
---

## [STEP 1] Router0 설정 (ISP 1 및 NAT)

Router0은 외부 ISP 1라인(111.111.111.2)을 처리하며, 내부망의 인터넷 통신을 지원합니다.
...
Router0(config-if)# ip nat outside             ! 외부 인터페이스 선언
...
Router0(config-if)# ip nat inside              ! 내부 인터페이스 선언
...
Router0(config-router)# default-information originate ! 인터넷 경로 전파
...
## [STEP 2] Router1 설정 (ISP 2 및 로드밸런싱)

Router1은 ISP 2라인(222.222.222.2)을 통해 트래픽을 분산 처리합니다.
...
SW0은 모든 VLAN의 메인 게이트웨이 역할을 수행하도록 **Priority 110**을 적용합니다.

```bash
Switch> enable
Switch# configure terminal
SW0(config)# hostname SW0
SW0(config)# ip routing                      ! L3 라우팅 활성화 필수
...
! --- 다중 VLAN(10~120) HSRP Active 설정 ---
! * VLAN 10 예시 (나머지 VLAN 20~120도 동일한 패턴으로 반복)
SW0(config)# vlan 10
...
SW0(config-if)# standby 10 priority 110      ! Active 권한 부여
SW0(config-if)# standby 10 preempt           ! 우선순위 높은 장비가 Active 선점
SW0(config-if)# no shutdown
SW0(config-if)# exit
...
## [STEP 4] SW1 설정 (VLAN 10-120 Standby)

SW1은 장애 대비용으로 기본 우선순위(100)를 유지합니다.
...
! --- 다중 VLAN(10~120) HSRP Standby 설정 ---
SW1(config)# vlan 10
...
* **복사 방법**: `!`로 시작하는 주석을 제외하고 명령어를 순서대로 복사하여 CLI 창에서 마우스 우클릭 -> **Paste**를 누르세요.
* **통신 확인**: PC에서 `ping 8.8.8.8` 또는 `ping 111.111.111.1`을 통해 인터넷 연결을 확인합니다.
* **상태 확인**: 스위치에서 `show standby brief`를 입력하여 HSRP 상태(Active/Standby)가 정상인지 체크합니다.
* **AWS 과금 주의**: 실제 환경에서 AWS VPN을 구성할 경우 터널당 시간 비용(약 $0.05/hr)이 발생하므로 실습 후 반드시 자원을 삭제해야 합니다.

Next Level : AWS Transit Gateway와 BGP를 이용한 다중 VLAN 라우팅 통합 분산

복사 붙여넣기 스크립트, no shutdown 포함, HSRP Priority 110, OSPF ECMP 활성화
